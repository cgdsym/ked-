...
if __name__ == "__main__":
    ...
    num_layers=3     # decoder的层数
)
    #ked_encoder的输出 [batch, 768, 157] 需交换位置
    ecg_features = torch.randn(8, 157, 768)       # batch=8, seq_len=157, embed_dim=768

    # 标签特征由文本编码器（如BERT）编码，假设有5个标签
    label_features = torch.randn(5, 768)          # num_labels=5, embed_dim=768

    # 3. 前向推理
    out = model(ecg_features, label_features)

    print(out.shape)  # [8, 5, 2]，对应 batch=8, num_labels=5, 二分类