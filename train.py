...
from llava.constants import IGNORE_INDEX, ECG_TOKEN_INDEX, DEFAULT_ECG_TOKEN, DEFAULT_ECG_START_TOKEN, \
    DEFAULT_ECG_END_TOKEN
...
class ModelArguments:
    ...
    mm_use_ecg_start_end: bool = field(default=False)
    mm_use_ecg_patch_token: bool = field(default=True)
...
def find_all_linear_names(model):
    ...
    multimodal_keywords = ['mm_projector', 'ecg_tower', 'ecg_resampler']
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        ...
        if getattr(trainer.args, "use_ecg_start_end", False):
        ...
        for sentence in source:
            if DEFAULT_ECG_TOKEN in sentence['value']:
                ...
                if "mmtag" in conversation_lib.default_conversation.version:
                    sentence['value'] = sentence['value'].replace(DEFAULT_ECG_TOKEN,
                                                                  '<ecg>' + DEFAULT_ECG_TOKEN + '</ecg>')
            replace_token = DEFAULT_ECG_TOKEN
            if data_args.mm_use_ecg_start_end:
                replace_token = DEFAULT_ECG_START_TOKEN + replace_token + DEFAULT_ECG_END_TOKEN
            sentence["value"] = sentence["value"].replace(DEFAULT_ECG_TOKEN, replace_token)
...
def preprocess_llama_2(
    ...
    if has_ecg:
        input_ids = torch.stack(
            [tokenizer_ecg_token(prompt, tokenizer, return_tensors='pt') for prompt in conversations], dim=0)
    else:
        input_ids = tokenizer(
            conversations,
            return_tensors="pt",
...
def preprocess_mpt(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(re_rounds):
            ...
            if has_ecg:
                round_len = len(tokenizer_ecg_token(rou, tokenizer))
                instruction_len = len(tokenizer_ecg_token(parts[0], tokenizer)) - 1
            else:
                round_len = len(tokenizer(rou).input_ids)
...
def preprocess_plain(
    ...
    for source in sources:
        assert len(source) == 2
        assert DEFAULT_ECG_TOKEN in source[0]['value']
        source[0]['value'] = DEFAULT_ECG_TOKEN
        conversation = source[0]['value'] + source[1]['value'] + conversation_lib.default_conversation.sep
        conversations.append(conversation)
    # tokenize conversations
    input_ids = [tokenizer_ecg_token(prompt, tokenizer, return_tensors='pt') for prompt in conversations]
    targets = copy.deepcopy(input_ids)
    for target, source in zip(targets, sources):
        tokenized_len = len(tokenizer_ecg_token(source[0]['value'], tokenizer))
...
def preprocess(
    ...
    if conversation_lib.default_conversation.sep_style == conversation_lib.SeparatorStyle.LLAMA_2:
        return preprocess_llama_2(sources, tokenizer, has_ecg=has_ecg)
    if conversation_lib.default_conversation.version.startswith("v1"):
        return preprocess_v1(sources, tokenizer, has_ecg=has_ecg)
    if conversation_lib.default_conversation.version == "mpt":
        return preprocess_mpt(sources, tokenizer, has_ecg=has_ecg)
    # add end signal and concatenate together
    conversations = []
    for source in sources:
    ...
    def get_tokenize_len(prompts):
        return [len(tokenizer_ecg_token(prompt, tokenizer)) for prompt in prompts]
...
def train(attn_implementation=None):
    ...
    if model_args.ecg_tower is not None:
        ...
        model.config.mm_use_ecg_start_end = data_args.mm_use_ecg_start_end = model_args.mm_use_ecg_start_end
        model.config.mm_projector_lr = training_args.mm_projector_lr
        training_args.use_ecg_start_end = model_args.mm_use_ecg_start_end
        model.config.mm_use_ecg_patch_token = model_args.mm_use_ecg_patch_token
        model.initialize_ecg_tokenizer(model_args, tokenizer=tokenizer)
